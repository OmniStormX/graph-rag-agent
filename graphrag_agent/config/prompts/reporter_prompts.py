...
1. **结构清晰**: 每个段落一个中心论点
2. **证据充分**: 每个论断必须有证据支持
3. **引用规范**: 使用 `[[ref:关键词|result_id]]` 标注证据，如"孙悟空拜[[ref:菩提祖师|result_abc123]]为师"
4. **逻辑连贯**: 段落之间过渡自然
5. **语言专业**: 准确、简洁、避免口语化
6. **字数控制**: 接近预估字数，不要过度偏离
7. **禁止额外标题**: 章节正文内部不要再使用 `#`/`##`/`###` 等 Markdown 标题，不要重复章节标题，也不要提前输出结论类标题
8. **公式规范**: 若章节包含数学公式、状态方程、推导式或符号表达，必须统一使用 Markdown 数学公式格式，行内公式使用 `$...$`，独立公式使用 `$$...$$`
9. **变量解释**: 公式出现后，应在相邻句中解释主要符号、变量含义、单位或适用条件，避免只罗列公式

**引用格式**:
- 在需要引用的关键词、短语或核心术语上使用 `[[ref:关键词|result_id]]`
- `关键词` 必须是正文中实际出现的那一小段词语，不要额外再输出一遍引用 ID
- `result_id` 对应 `evidence_list` 中的 `RetrievalResult.result_id`
- 同一证据可以多次引用
- 禁止输出旧格式 `[result_id]`，除非上游系统明确要求兼容

**示例输出**:

孙悟空离开花果山后，历经千山万水来到斜月三星洞，拜[[ref:菩提祖师|result_node_03]]为师。在这一阶段，他开始接触到修行的纪律和天地间的规则。菩提祖师传授了他[[ref:七十二变和筋斗云|result_chunk_04]]等神通，这不仅让孙悟空获得了强大的能力，也第一次让他意识到力量的边界和责任。

然而，当孙悟空在师兄弟面前卖弄本领时，菩提祖师严厉惩罚并将其[[ref:逐出师门|result_chunk_05]]。这一事件成为孙悟空思想转变的起点，他开始质疑权威和规则的合理性。正如小说中所描述的：[[ref:他不明白为什么有了本事就不能用|result_quote_06]]。这种困惑为后来的反抗天庭埋下了伏笔。

从心理发展的角度看，这一阶段孙悟空经历了从无知到觉醒的关键转变。他开始[[ref:独立思考|result_analysis_07]]，不再盲从权威。这种觉醒不仅是个人成长的必然，也是《悟空传》主题，个体自由与制度约束冲突的核心体现。
...
**2. 引用准确性检查**:
- 所有引用标记 `[[ref:关键词|result_id]]` 是否都有对应的证据？
- 引用的内容是否准确反映了证据的原意？
- 是否有遗漏重要证据未引用？
...
**示例1 - 一致性良好**:
报告: "孙悟空拜[[ref:菩提祖师|result_node_01]]为师，学习了[[ref:七十二变|result_chunk_02]]。"
...
**示例2 - 需要修正**:
报告: "孙悟空拜[[ref:唐僧|result_node_01]]为师，学习了七十二变。"
