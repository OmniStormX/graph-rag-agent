...
7. **fluid_property_calc**: 流体热物性计算，适合已知两个状态量后计算焓、熵、密度、干度等定量结果
8. **reflection**: 对已完成任务进行质量校验或补充改进建议，通常依赖已有答案
...
7. 只能输出合法 JSON，所有键名和字符串必须使用 ASCII 双引号 `"`，禁止使用中文引号 `“”`、中文单引号 `‘’`、注释、解释文本和尾逗号
...
- 不要输出 ```json 代码块围栏之外的任何解释
- 不要输出中文标点形式的 JSON 键值分隔符
- 输出前请自行检查 JSON 能否被标准 `json.loads` 解析
